...
class LabeledDataset(Dataset):
    ...
    def __init__(self,
        ...
        Args:
            ...
            small_dataset_percentage: Fraction of the dataset to use (0.0-1.0)
        """
        super().__init__()
        ...
        # Validate small_dataset_percentage is between 0 and 1
        if small_dataset_percentage is not None:
            if small_dataset_percentage <= 0 or small_dataset_percentage > 1.0:
                logger.warning(f"Invalid small_dataset_percentage: {small_dataset_percentage}. Must be between 0 and 1. Using 1.0 (full dataset).")
                self.small_dataset_percentage = 1.0
            else:
                self.small_dataset_percentage = small_dataset_percentage
        else:
            self.small_dataset_percentage = 1.0

        # Log if using a subset of the data
        if self.small_dataset_percentage < 1.0:
            logger.info(f"Using only {self.small_dataset_percentage:.1%} of the dataset for quick testing")

        # Set random seed for reproducibility - do this early to ensure consistent sampling
    ...
    def _find_matching_pairs(self):
        ...
        # Apply small_dataset_percentage if less than 1.0
        if self.small_dataset_percentage < 1.0 and audio_label_pairs:
            original_count = len(audio_label_pairs)
            sample_size = max(1, int(original_count * self.small_dataset_percentage))

            if sample_size < original_count:
                # Use the random seed from the constructor for consistency
                # No need to set random.seed() here as it was already set in __init__

                # Sample a subset of the pairs
                indices = random.sample(range(original_count), sample_size)
                audio_label_pairs = [audio_label_pairs[i] for i in indices]
                logger.info(f"Using small dataset: sampled {len(audio_label_pairs)}/{original_count} pairs ({self.small_dataset_percentage:.1%})")

                # Ensure we have at least one pair
                if not audio_label_pairs:
                    logger.warning("No audio-label pairs after sampling! Using one random pair to avoid empty dataset.")
                    if original_count > 0:
                        random_idx = random.randint(0, original_count - 1)
                        audio_label_pairs = [audio_label_pairs[random_idx]]
