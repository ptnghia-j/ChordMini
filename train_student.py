...
def main():
    ...
    parser.add_argument('--find_unused_parameters', action='store_true',
                      help='Find unused parameters in DDP model')
    ...
    if args.distributed:
        ...
        if args.local_rank == -1:
            ...
            # Using distributed launch, set device based on local_rank
    ...
    if args.distributed:
        ...
        # Standard data loading for non-distributed training
        train_loader = synth_dataset.get_train_iterator(
            batch_size=batch_size,
            shuffle=True,
            num_workers=0,  # Force single worker for GPU optimization
    ...
    # Apply SyncBatchNorm if requested for distributed training
    ...
    if args.distributed:
        ...
        logger.info(f"Model wrapped with DDP, device_id={args.local_rank}")

    # Attach chord mapping to model (to the module if using DDP)
    if args.distributed:
        model.module.idx_to_chord = master_mapping
        logger.info("Attached chord mapping to model.module for correct MIR evaluation")
    else:
    ...
    if args.distributed:
        ...
        logger.info("Using standard StudentTrainer")
