...
def load_hmm_model(model_path, device):
    ...
    try:
        ...
        # Check if pretrained_path is a relative path inside ChordMini
        if not os.path.isabs(pretrained_path) and pretrained_path.startswith('./'):
            # Try external storage path first
            external_path = pretrained_path.replace('./checkpoints/', '/mnt/storage/checkpoints/')
            if os.path.exists(external_path):
                logger.info(f"Using external checkpoint at {external_path}")
                pretrained_path = external_path
